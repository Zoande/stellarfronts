...
import type { CleanupRegistry } from "./CleanupRegistry";
import type {
  ClientCommand,
...
export async function startGameUi(
  ...
  const setSelectedFleetIds = (fleetIds: Iterable<string>): void => {
    ...
    controller.store.setSelectedFleetIds(selectedFleetIds);
  };
  ...
  async function openGalaxyView(): Promise<void> {
    controller.store.setView("galaxy");
  ...
  async function openSystemView(star: StarData): Promise<void> {
    controller.store.setView("system", star.id);
  ...
  async function openFirstHabitedPlanetFromGalaxy(starId: number): Promise<void> {
    ...
    controller.store.materializeSystemDetails(systemDetails);
  ...
  server.onSnapshot((nextSnapshot, changed) => {
    snapshot = controller.store.getSnapshot() ?? nextSnapshot;
